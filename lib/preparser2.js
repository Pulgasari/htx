...
// turns htx shorthand into something a browser can actually parse. The DOM
// work (definitions, custom-tag expansion, mounting) lives in htx.js.
//
// Performance notes:
...
function finishImplicitDivOpening(
  code,
  start,
  pos,
  id,
  ...
  transformAttributes(attrsWithoutSlash, parts);

  if (selfClosing) {
    return {
      output: `${parts.join('')}></div>`,
      end: pos + 1
    };
  }
...
function parseImplicitDivClosingTag(code, start, hashPos) {
  const length = code.length;
  ...
  // Optional .class / #id Selector konsumieren.
  // Für das erzeugte Closing-Tag sind sie nicht relevant.
  while (pos < length) {
    const marker = code.charCodeAt(pos);
  ...
  // Whitespace vor >
  while (pos < length && isSpace(code, pos)) {
    pos++;
  }

  if (code.charCodeAt(pos) !== CHAR_GT) {
    return null;
  }
