...
The core of htx: a fork of [htm](https://github.com/developit/htm) with merged
props, tag selectors, prop groups and shorthand tags. It has no renderer of its
own — pick an adapter:

```javascript
import { html } from '@htx/js';      // vanilla dom
import { html } from '@htx/preact';  // preact
```
...
`@htx/htx/markdown` walks a token tree into `h()` calls, so Markdown arrives as nodes
...
```javascript
import { createHTX } from '@htx/htx';

export const html = createHTX(h, Fragment, { memo: true, tags: {} });
```

`h(type, props, ...children)` is called bottom up. `memo` defaults to `true`,
which is what a vdom wants. `tags` seeds the shorthand registry. `createHtml`
is an alias of `createHTX`.
